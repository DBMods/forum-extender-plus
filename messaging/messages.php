		...
		<div id='wrapper'>
			...
			if ($_POST['for']) {
				...
				while ($row = mysql_fetch_assoc($result)) {
					...
					echo '<form method="post" action="messages.php" class="menu"><input type="hidden" name="timeOffset" value="' . htmlspecialchars($_POST['timeOffset']) . '" /><input type="hidden" name="action" value="compose" /><input type="hidden" name="returnto" value="' . $_POST['returnto'] . '" /><input name="context" type="hidden" value="' . stripslashes($row['msg']) . '"/><input name="to" type="hidden" value="' . htmlspecialchars($row['from']) . '" /><input type="hidden" name="from" value="' . htmlspecialchars($_POST['for']) . '" /><button type="submit">Reply</button></form>';
