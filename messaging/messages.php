		...
		<div id='wrapper'>
			...
			if ($_POST['for']) {
				...
				while ($row = mysql_fetch_assoc($result)) {
					...
					echo 'Time: ' . $row['time'] . 'From: <a href="https://forums.dropbox.com/profile.php?id=' . $row['from'] . '" target="_blank">' . $row['from'] . '</a><br>Message:<br>' . htmlspecialchars_decode(stripslashes($row['msg'])) . '</p>';
				}
			}
			mysql_close($db);
			?>
