		...
		<div id='wrapper'>
			...
			if ($_POST['for']) {
				...
				while ($row = mysql_fetch_assoc($result)) {
					...
					echo '<br>Time: ' . gmdate('Y-m-d g:i A', $row['time'] - $timeOffsetSeconds) . '<br>From: <a href="https://forums.dropbox.com/profile.php?id=' . $row['from'] . '" target="_blank">' . $row['from'] . '</a><br>Message:<br>' . nl2br(htmlspecialchars_decode(stripslashes($row['msg'])));
					echo '</p>';
				}
			}
			mysql_close($db);
