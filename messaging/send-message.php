...
if (is_numeric($dest) && $dest != 0) {
	...
	echo '<p>Sorry, invalid destination.<br><form class="menu" action="messages.php" method="post"><input type="hidden" name="returnto" value="' . strip_tags($_POST['returnto']) . '" /><input name="action" type="hidden" value="compose" /><input name="to" type="hidden" value="' . $dest . '"/><br><input type="hidden" name="msgtext" value="' . htmlspecialchars($_POST['msgtext']) . '" /><br><button type="submit">Go Back</button></form></p>';
?>
