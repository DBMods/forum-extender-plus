...
if ($('#topic_labels:contains("[sticky]")').length == 0 && $('#topic-info .topictitle:contains(") - ")').length > 0 && $('#topic-info .topictitle:contains(" Build - ")').length > 0) {
	var threadType = $('#topic-info .topictitle:contains(") - ")').html().split(') - ')[1].split(' Build - ')[0];
	showModal(['Yes', 'No'], 'Find newer sticky?', 'This thread is no longer sticky. Would you like to attempt to find the latest ' + threadType.toLowerCase() + ' build thread?', function() {
		GM_xmlhttpRequest({
			method: 'GET',
			...
			onload: function(response) {
				...
				var stickies = [];
				$(resp).find('td:contains("' + threadType + '") big a').each(function() {
					stickies.push($(this).attr('href'));
				});
				if (stickies)
					window.location.href = stickies[0];
			}
		});
	});
}
...
if (client.isAuthenticated()) {
	client.getDatastoreManager().openDefaultDatastore(function(error, datastore) {
		...
		$('.gsDropboxExtenderSignatureInsert').click(function() {
			...
			insertTextAtCursorPosition("\n\n--\n" + (sig.length > 0 ? sig[0].get('value') : 'You haven\'t set your signature yet. Head over to the preferences menu to do that.'));
		});
		...
		function reloadPage(pageType) {
			...
			if (reloadIndex[pageType] && reloadDelay.length > 0 && reloadDelay[0].get('value') > 0) {
				setTimeout(function() {
					if (!modalOpen && !$('#post_content').val())
						document.location.reload();
					else
						reloadPage(pageType);
		...
		if (pageUrl == 'topic.php' || pageUrl == 'edit.php') {
			$('#postform div.clear').append('<p id="gsDropboxExtenderPostExtras" style="float:left;margin-right:20px"><select id="snippets"><option name="default" value="">' + (snippets.length > 0 ? 'Select a snippet' : 'You don\'t have any snippets') + '</option><optgroup label="--Snippets--" /></select></p>');
		...
		$('#postform div.clear').append($('#post-form-allowed-container'));
		...
		*/
		...
		//Collapse footer
		if ($(window).height() + $('#footer').height() < $(document).height() && collapseFooter.length > 0 && collapseFooter[0].get('value')) {
			//Style footer
			$('#footer').css({
		...
		$('.threadauthor small a:contains("Super User")').parent().parent().find('strong').find('img').attr('src', modIcon.length ? modIcon[0].get('value') : 'https://forum-extender-plus.s3-us-west-2.amazonaws.com/icons/nyancatright.gif');
	});
} else {
...
function getPostAuthorDetails(postEventTarget) {
	...
	return '<a href="' + fullUrl.split('#')[0] + $(postEventTarget).parent().find('a:contains("#")').attr('href') + '"><strong>' + ($(stuff).find('a').html() || $(stuff).html()) + '</strong> scribbled:</a>\n';
}
...
function showModal(buttons, title, content, action, actionTwo) {
	...
	for (var i = 0; i < buttons.length; i++) {
		$('#gsDropboxExtenderModalActionButtons').append('<a href="javascript:void(0);" class="button gsDropboxExtenderModal' + buttonMap[buttons[i]] + '" style="margin-left:18px">' + buttons[i] + '</a>');
	}

	$("#gsDropboxExtenderModal").css({
		"top": (document.documentElement.clientHeight - $('#gsDropboxExtenderModal').height()) / 2,
		"left": (document.documentElement.clientWidth - $('#gsDropboxExtenderModal').width()) / 2
	});
