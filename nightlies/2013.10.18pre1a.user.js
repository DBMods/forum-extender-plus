...
//Set up alerts
var alertSummary = {
	'summary': 'New reskin resulting in problems - fixed',
	'fullDesc': 'Dropbox\'s new forum reskin has a few aesthetic bugs in it. I\'m trying to add to the script to fix this as fast as I can. In the meantime, it\'s a work in progress, and won\'t affect usability.<br><br>It looks to be fixed, but I\'m still in the process of debugging and testing the fix to make sure there are no holes. Sit tight!'
};
...
//Calculate post streaks
function postStreak(){
	if(pageUrl == 'topic.php'){
		$('#post-form-submit-container').html('<span id="postformsub" class="freshbutton-blue">Send Post &raquo;</span>');
		$('#postformsub').click(function(){
			if($('#post_content').val() != ''){
				var d = new Date();
				var day = d.getDate();
				var streak = GM_getValue('currentstreak', 0), longest = GM_getValue('longeststreak', 0), streakDate = GM_getValue('streakdate', 0);
				...
				GM_setValue('currentstreak', streak);
				GM_setValue('longeststreak', longest);
				GM_setValue('streakdate', streakDate);
...
//Manage post drafts TODO: Merge into nav bar
function draftPost() {
    if(pageUrl == 'topic.php') {
...
function navBar() {
	...
	$('#modicon-nav').append('<span>Current Post Streak: ' + streak + '</span><span>Best Streak: ' + longest + '</span>');
	if(pageUrl == 'topic.php'){
		$('#post-form-submit-container').html('<span id="postformsub" class="freshbutton-blue">Send Post &raquo;</span>');
		$('#postformsub').click(function(){
			if($('#post_content').val() != ''){
...
function reloadPage(pageType) {
    ...
    if(reloadIndex[pageType] && reloadDelay > 0) {
        setTimeout(function() {
            if(!settingsVisible && (pageUrl == 'topic.php') ? (!$('#topic').val() && !$('#post_content').val()) : true)
